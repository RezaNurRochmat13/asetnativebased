...
<html>
  ...
  $FC = new FusionCharts("column2d","500","250");
 ...
 //$strParam="caption=Grafik Jumlah Inventaris dan Aset Berdasarkan Kategori ";
  if ($filter == "All" or $filter == "") {
    $strParam="caption=Grafik Jumlah Aset keseluruhan ; exportEnabled=1;exportHandler=http://localhost/aino/aset/ExportChart/Handlers ; exportAtClient=0 ; exportAction=download ; exportTargetWindow=_blank ; exportFileName=MyFileName ";
  }
  else 
  {
    if ($bulan == "-01-") {
     ...
     $strParam="caption=Grafik Jumlah Aset $filter dari status barang $status sampai pada bulan $bulan tahun $tahun ";
  }
